...
import type { ChatScreenProps, ConversationItem } from "@/types";
...
export const ChatScreen = ({
    ...
    const userInitials =
        chatUser?.displayName
            ?.split(" ")
            .map((n: string) => n[0])
            .join("") || mockUserInitials;
    const userAvatar =
        chatUser?.avatarUrl || chatUser?.avatar || mockUserAvatar;
    ...
    return (
        <View style={styles.screen}>
            ...
            >
                ...
                {(conversation as ConversationItem[]).map((item) => (
                    <View
                        key={item.id}
                        style={[
                            styles.bubbleRow,
